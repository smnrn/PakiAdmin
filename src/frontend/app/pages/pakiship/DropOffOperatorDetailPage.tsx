...
import {
  MOCK_OPERATORS,
  type DropOffHistoryItem,
  type DropOffOperatorRecord,
  type OperatorAccountStanding,
  type OperatorStatus,
} from './dropOffOperatorRecords';
...
export default function DropOffOperatorDetailPage({ operatorId }: DropOffOperatorDetailPageProps) {
  ...
  const initialOperator = MOCK_OPERATORS.find((item) => item.id === operatorId) ?? MOCK_OPERATORS[0];
  const [operator, setOperator] = useState<DropOffOperatorRecord>(initialOperator);
  ...
  const handleConfirmAccountAction = () => {
    ...
    const nextStanding: OperatorAccountStanding =
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-10 space-y-8">
          ...
          <section className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            ...
            <MetricCard icon={<Star className="h-5 w-5" />} label="Customer Rating" value={operator.averageRating.toFixed(1)} detail={`Latest ${latestRating.rating.toFixed(1)} rating`} tone="amber" />
          </section>
          ...
          <section className="grid grid-cols-1 gap-6 xl:grid-cols-[0.85fr_1.15fr]">
            <div className="rounded-[2rem] border border-[#39B5A8]/10 bg-white p-6 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm font-semibold text-[#1A5D56]/60">{capacity.utilizationRate}% utilized</p>
                </div>
                <span className="rounded-full border border-[#39B5A8]/10 bg-[#F0F9F8] px-3 py-1 text-xs font-black text-[#39B5A8]">
                  {capacity.availableBins} Available
                </span>
              </div>

              <div className="mt-6">
                <div className="h-3 overflow-hidden rounded-full bg-[#F0F9F8]">
                  <div className="h-full rounded-full bg-[#39B5A8]" style={{ width: `${capacity.utilizationRate}%` }} />
                </div>
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <CapacityTile label="Total Bins" value={capacity.totalBins.toLocaleString()} />
                  <CapacityTile label="Occupied" value={capacity.occupiedBins.toLocaleString()} />
                  <CapacityTile label="Reserved" value={capacity.reservedBins.toLocaleString()} />
                  <CapacityTile label="Available" value={capacity.availableBins.toLocaleString()} />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-[#39B5A8]/10 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 space-y-3">
                {operator.customerRatings.map((item) => (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-[#39B5A8]/5">
                  {operator.dropOffHistory.map((item) => (
...
function HistoryStatusBadge({ status }: { status: DropOffHistoryItem['status'] }) {
  const statusConfig = {
...
function AccountStandingBadge({ standing }: { standing: OperatorAccountStanding }) {
